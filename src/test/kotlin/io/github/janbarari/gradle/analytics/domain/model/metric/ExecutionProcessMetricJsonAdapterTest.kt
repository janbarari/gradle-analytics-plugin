...
class ExecutionProcessMetricJsonAdapterTest {

    lateinit var moshi: Moshi
    lateinit var adapter: ExecutionProcessMetricJsonAdapter

    @BeforeAll
    fun setup() {
        moshi = Moshi.Builder().build()
        adapter = ExecutionProcessMetricJsonAdapter(moshi)
    }
    ...
    fun `Check toJson() return valid Json with valid data model`() {
        val validModel = ExecutionProcessMetric(
            median = 1000L,
            mean = 1200L
        )
        assertDoesNotThrow {
