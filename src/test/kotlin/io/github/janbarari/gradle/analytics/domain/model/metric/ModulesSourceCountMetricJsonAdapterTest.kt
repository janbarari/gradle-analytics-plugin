...
class ModulesSourceCountMetricJsonAdapterTest {

    lateinit var moshi: Moshi
    lateinit var adapter: ModulesSourceCountMetricJsonAdapter

    @BeforeAll
    fun setup() {
        moshi = Moshi.Builder().build()
        adapter = ModulesSourceCountMetricJsonAdapter(moshi)
    }
    ...
    fun `Check toJson() return valid Json with valid data model`() {
        val validModel = ModulesSourceCountMetric(
            modules = emptyList()
        )
        assertDoesNotThrow {
