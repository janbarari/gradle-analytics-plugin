...
class InitializationProcessMetricJsonAdapterTest {

    lateinit var moshi: Moshi
    lateinit var adapter: InitializationProcessMetricJsonAdapter

    @BeforeAll
    fun setup() {
        moshi = Moshi.Builder().build()
        adapter = InitializationProcessMetricJsonAdapter(moshi)
    }
    ...
    fun `Check toJson() return valid Json with valid data model`() {
        val validModel = InitializationProcessMetric(
            median = 1000L,
            mean = 1200L
        )
        assertDoesNotThrow {
