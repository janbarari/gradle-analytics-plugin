...
class ModulesMethodCountReportJsonAdapterTest {

    lateinit var moshi: Moshi
    lateinit var adapter: ModulesMethodCountReportJsonAdapter

    @BeforeAll
    fun setup() {
        moshi = Moshi.Builder().build()
        adapter = ModulesMethodCountReportJsonAdapter(moshi)
    }
    ...
    fun `Check fromJson() returns valid data model with valid json`() {
        ...
        assertTrue {
            fromReader.totalMethodCount == 100
        }
        assertTrue {
    ...
    fun `Check toJson() return valid Json with valid data model`() {
        val validModel = ModulesMethodCountReport(
            values = emptyList(), totalDiffRate = 0f, totalMethodCount = 0
        )
        assertDoesNotThrow {
