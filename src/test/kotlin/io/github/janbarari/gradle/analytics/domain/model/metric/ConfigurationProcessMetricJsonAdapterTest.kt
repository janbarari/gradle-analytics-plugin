...
class ConfigurationProcessMetricJsonAdapterTest {

    lateinit var moshi: Moshi
    lateinit var adapter: ConfigurationProcessMetricJsonAdapter

    @BeforeAll
    fun setup() {
        moshi = Moshi.Builder().build()
        adapter = ConfigurationProcessMetricJsonAdapter(moshi)
    }
    ...
    fun `Check toJson() return valid Json with valid data model`() {
        val validModel = ConfigurationProcessMetric(
            median = 1000L,
            mean = 1200L
        )
        assertDoesNotThrow {
